...
The number of AP patients in the dataset is `r count_ap` and the number of nonAP
patients is `r count_nonap`.
...
patients for both conditions (AP vs. nonAP) and for the diagnosis/treatment
...
p <- ggplot(countlabel, aes(x = variable, y= value, shape = label, colour = label)) +
  geom_point(stat='identity') + theme(axis.text.x = element_text(angle = 90)) +
  xlab("Patient ID") + ylab ("Count") + ggtitle("Cell count per patient")
