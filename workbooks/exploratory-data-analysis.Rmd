...
From our Q-Q plot and the Shapiro-Wilk test above we see that the cell count per 
sample is not normally distributed.

In a last step the proportion of AP cells vs nonAP cells over all samples
is analysed.
```{r, message = FALSE}
...
In the Q-Q plot we clearly see that the data is not
normally distributed, and the Shapiro-Wilk test gives a p-value $\geq 0.05$ 
motivating the use of non-parametric test in downstream analyses.
