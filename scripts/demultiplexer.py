...
def main():
    for folder in os.listdir("../data/"):
            if "preprocessed" not in folder:
                with open(f"../data/{folder}/outs/filtered_contig.fasta", "r") as input:
                    for line in input:
                        if line.startswith(">"):
                        ...
                        with open(f"../data/{folder}-preprocessed/{cell}.fasta", "a") as output:
                            output.write(line)

                with open(f"../data/{folder}-preprocessed/list_of_cells.txt", "w") as file:
