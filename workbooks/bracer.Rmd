...
With these demultiplexed cells one could run the BraCeR workflow with the two
functionalities `assemble` and `summarise`.
...
To reduce folder size, only the files containing summarized information useful
for the downstream analysis were selected an packaged. This was done with the
script below.
...
We therefore have two relevant files for our downstream analysis,
`BCR_summary.txt` and `clonotype_sizes.txt`. The former contains a summary
statistics describing successful BCR reconstruction rates and the numbers of
cells with 0, 1, 2 or more recombinants for each locus, and the latter contains
the distribution of clonotype sizes as a tabulated text file. Other files were
generated but not used; refer to the [bracer
