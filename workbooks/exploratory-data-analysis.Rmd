...
Next we want to inspect how many cells we have per sample.
...
We see that we have in the range of cell counts in the range of $\sim$ 1000 up to $\sim$ 5000. Over the two conditions AP and nonAP we see that the distribution is quite similar with `r format(cells_ap, scientific=FALSE)` AP cells and `r format(cells_nonap, scientific=FALSE)` nonAP cells, which again points to a well balanced dataset.
...
p <- ggplot(summary.df, aes(x = "", y = value, fill = Type)) + geom_bar(stat = "identity") + xlab ("Cumsum of Cells") + ggtitle("Proportion of overall AP vs nonAP cells")
p
```
